...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetButtonDown("Jump") && JumpCountDown > 0)
        {
            JumpCountDown -= 1;
            jumpRequest = true;
            if(JumpCountDown == 0)
    ...
    private void JumpUpdate()
    {
        ...
        if (jumpRequest)
        {
            //if(!isGrounded && JumpCountDown == 1)
            //{
                //playerRb.AddForce(Vector2.up * jumpForce * 15.0f, ForceMode2D.Impulse);
                //playerRb.velocity = new Vector2(playerRb.velocity.x, 100.0f);
            //}
